...
class MasterViewController: UITableViewController {
    ...
    func insertNewObject(sender: AnyObject) {
        if let nowPlayingItem = getNowPlayingItem(){
            let playbackDuration = nowPlayingItem.playbackDuration
            nowPlayingItem.mediaType
            print("Current playback time:\(getCurrentPlaybackTime())\nAlbum artist:\(nowPlayingItem.albumArtist ?? "")\nTitle:\(nowPlayingItem.title ?? "")\n\(playbackDuration)")
            objects.insert("\(nowPlayingItem.albumArtist ?? "") - \(nowPlayingItem.title ?? "")", atIndex: 0)
